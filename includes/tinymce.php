 ...
 * @since 2.0.0
 */
class CCEssentials_TinyMCE {
	/**
	 * The one instance of CCEssentials_TinyMCE.
	 *
	 * @since 2.0.0.
	 *
	 * @var CCEssentials_TinyMCE
	 */
	private static $instance;

	/**
	 * Instantiate or return the one CCEssentials_TinyMCE instance.
	 *
	 * @since  2.0.0.
	 *
	 * @return CCEssentials_TinyMCE
	 */
	...
	public function style_formats( $settings ) {
		...
		$style_formats = apply_filters( 'cce_style_formats', $style_formats );
	...
	public function tiny_mce_before_init( $mceInit, $editor_id ) {
		if ( ! empty( $mceInit['toolbar1'] ) ) {
			if ( in_array( 'hr', explode( ',', $mceInit['toolbar1'] ) ) ) {
				...
				$mceInit['toolbar1'] = str_replace( ',hr,', ',cce_mce_hr_button,', $mceInit['toolbar1'] );
 ...
 * @return CCEssentials_TinyMCE
 */
function cce_get_tinmyce_styles() {
	return CCEssentials_TinyMCE::instance();
}

add_action( 'admin_init', 'cce_get_tinmyce_styles' );
